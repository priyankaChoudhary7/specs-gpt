...
import time
import shutil # For clearing the Chroma DB directory
...
def clear_chroma_db(directory):
    ...
    # Ensure the directory exists for Chroma to initialize
...
# The vector_store should ideally be recreated or cleared for each analysis
# So, we won't cache the vector_store itself, but rather the way to get a new one.
def get_new_vector_store(_embeddings):
    # Clear DB before creating a new instance to ensure a clean slate
...
def run_analysis_for_section(rag_chain, section_title, details):
    # Construct a more focused query for each section
    # Use keywords directly related to the section for retrieval
    search_query = f"{details['keywords']} for {section_title}"
    ...
    try:
        ...
        st.error(f"Error during analysis for {section_title}: {str(e)}")
        return section_title, f"Error during analysis: {str(e)}"

# --- MODIFIED FUNCTION WITH TIMERS AND CONSISTENCY IMPROVEMENTS ---
def process_and_analyze_pdf(pdf_file, vector_store, llm, embeddings):
    timings = []
    pdf_filename = pdf_file.name
    
    # Step 0: Ensure a clean Chroma DB for this analysis
    # This is handled in get_new_vector_store, but mentioning here for clarity
    ...
    # Step 2: Chunking and Document Preparation
    with st.status("Step 2/4: Chunking document...", expanded=True) as status:
        s2_start = time.perf_counter()
        # Fine-tune these parameters for optimal performance and consistency
        # Consider a larger overlap to ensure context around breaks
        text_splitter = RecursiveCharacterTextSplitter(chunk_size=1500, chunk_overlap=300) 
        chunks = text_splitter.split_text(full_text)
        # Assign a consistent source metadata for filtering
    ...
    # Step 3: Embedding Generation and Storing in Chroma
    with st.status("Step 3/4: Creating semantic embeddings and storing in vector database...", expanded=True) as status:
        s3_start = time.perf_counter()
        # Use the passed-in vector_store (which is already cleared)
        vector_store.add_documents(documents)
        # vector_store.persist() # Explicitly persist after adding documents
    ...
    with st.status("Step 4/4: Analyzing all document sections in parallel...", expanded=True) as status:
        ...
        # Increase 'k' for broader retrieval and consider 'search_type="mmr"' for diversity
        # MMR (Maximal Marginal Relevance) helps in getting diverse yet relevant results
        ...
        report = {}
        with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor: # Reduced max_workers to manage resources
...
# st.markdown("**:orange[Note:]** The system clears its internal memory (Chroma DB) with each analysis to ensure consistent results from a fresh start.")
...
if uploaded_file is not None:
    if st.button(f"Analyze {uploaded_file.name}", help="Click to start the analysis. This will clear previous data for consistency."):
        st.session_state.report = None
        st.session_state.timings = None
        # Create a new, cleared vector store for each analysis
        vector_store = get_new_vector_store(embeddings) 
        report_data, timings_data = process_and_analyze_pdf(uploaded_file, vector_store, llm, embeddings)
...
if st.session_state.report:
    ...
    for division, sections in sorted_divisions:
        with st.expander(f"**{division}**", expanded=True):
            ...
            for section_title, analysis in sorted_sections:
                ...
                if "Information not found." in analysis:
                    st.markdown(f"<span style='color:red;'>{analysis}</span>", unsafe_allow_html=True)
                else:
     ...
     # --- DOWNLOAD PDF BUTTON ---
    def generate_pdf(report_data, timings_data, filename="specification_analysis_report.pdf"):
        buffer = io.BytesIO()
        ...
        for division, sections in sorted(report_data.items()):
            ...
            for section_title, analysis in sorted(sections.items()):
                ...
                # You might also want to apply red highlighting in the PDF, which requires more ReportLab specifics
                # For now, it will just be regular text in PDF.
                # If "Information not found." in analysis:
                #    # ReportLab styling for color would go here, e.g., using a custom style or inline tags
                #    # This is more complex for ReportLab and out of scope for a quick fix.
    ...
    pdf_bytes = generate_pdf(st.session_state.report, st.session_state.timings, uploaded_file.name.replace(".pdf", "_report.pdf"))
    st.download_button(
